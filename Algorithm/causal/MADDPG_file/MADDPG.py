...
# 避免首次创建 Adam 等时懒加载 torch._dynamo → 分布式算子链，在部分 Windows 环境极慢或像卡死
...
from mpe_env import import_mpe_module
...
def _current_exploration_scale(args, supplement, ou_noise, env_agents):
    if supplement['OUNoise'] and ou_noise is not None:
        return float(ou_noise[env_agents[0]].scale)
    return float(args.gauss_scale)


def _open_trajectory_writer(model_dir, env_agents, dim_info, traj_format: str = "default"):
    """
    traj_format:
      - default: 每个环境步一行；公共列 episode/global_step/episode_step，各 agent 一列 JSON
                 → training_trajectory.csv（单步内各智能体独占一列字典串）
      - main_like: 每个 agent 一步一行；扁平列 s_* / dw / done → training_trajectory_main_like.csv
      - json_per_step: 与 default **同结构**，文件名 training_trajectory_agents_json.csv（便于并行保留两份）
    """
    if traj_format in ("default", "json_per_step"):
        fname = (
            "training_trajectory_agents_json.csv"
            if traj_format == "json_per_step"
            else "training_trajectory.csv"
        )
        traj_path = os.path.join(model_dir, fname)
        traj_fp = open(traj_path, "w", newline="", encoding="utf-8-sig")
        header = ["episode", "global_step", "episode_step"] + [str(aid) for aid in env_agents]
        writer = csv.writer(traj_fp)
        writer.writerow(header)
        tag = "(JSON columns per agent)" if traj_format == "default" else "(JSON columns, alias file)"
        print(f"Training trajectory CSV {tag}:", os.path.abspath(traj_path))
        return traj_fp, writer, traj_format

    agent_id = env_agents[0]
    obs_dim = dim_info[agent_id][0]
    act_dim = dim_info[agent_id][1]
    fname = "training_trajectory_main_like.csv"
    traj_path = os.path.join(model_dir, fname)
    traj_fp = open(traj_path, "w", newline="", encoding="utf-8-sig")
    header = ["episode", "global_step", "episode_step", "agent_id"]
    header += [f"action_{i}" for i in range(act_dim)]
    header += ["reward", "dw", "truncated", "done"]
    header += [f"s_{i}" for i in range(obs_dim)]
    header += [f"s_next_{i}" for i in range(obs_dim)]
    writer = csv.writer(traj_fp)
    writer.writerow(header)
    print("Training trajectory CSV:", os.path.abspath(traj_path))
    return traj_fp, writer, traj_format


def _agent_transition_json(agent_id, dim_info, action, reward, obs, next_obs, terminated, truncated):
    """单智能体一步转移，序列化为可被 json.dumps 的结构。"""
    term = bool(terminated[agent_id])
    trunc = bool(truncated[agent_id])
    obs_arr = np.asarray(obs[agent_id], dtype=np.float64).reshape(-1)
    na = np.asarray(action[agent_id], dtype=np.float64).reshape(-1)
    nx = np.asarray(next_obs[agent_id], dtype=np.float64).reshape(-1)
    return {
        "agent_id": str(agent_id),
        "obs_dim": int(dim_info[agent_id][0]),
        "action_dim": int(dim_info[agent_id][1]),
        "obs": obs_arr.tolist(),
        "action": na.tolist(),
        "reward": float(reward[agent_id]),
        "terminated": term,
        "truncated": trunc,
...
def _write_trajectory_rows(
    ...
    if traj_format in ("default", "json_per_step"):
        ...
        for agent_id in env_agents:
            payload = _agent_transition_json(
                agent_id,
                dim_info,
                action,
                reward,
            ...
            row.append(json.dumps(payload, ensure_ascii=False, separators=(",", ":")))
        traj_writer.writerow(row)
        return

    for agent_id in env_agents:
        row = [episode_idx, global_step, episode_step, agent_id]
        row.extend(float(x) for x in np.asarray(action[agent_id]).reshape(-1))
        row.append(float(reward[agent_id]))
        term = int(bool(terminated[agent_id]))
        trunc = int(bool(truncated[agent_id]))
        row.append(term)
        row.append(trunc)
        row.append(int(bool(term or trunc)))  # done：本步后是否结束该回合（与 Buffer 一致）
        row.extend(float(x) for x in np.asarray(obs[agent_id]).reshape(-1))
        row.extend(float(x) for x in np.asarray(next_obs[agent_id]).reshape(-1))
        traj_writer.writerow(row)


def _save_training_episodes_csv(model_dir, episode_rows, env_agents):
    ep_path = os.path.join(model_dir, 'training_episodes.csv')
    fieldnames = ['episode', 'global_step', 'mean_reward', 'exploration_scale']
    fieldnames += [f'reward_{agent_id}' for agent_id in env_agents]
    with open(ep_path, 'w', newline='', encoding='utf-8-sig') as fp:
        writer = csv.DictWriter(fp, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(episode_rows)
    print('Training episodes CSV:', os.path.abspath(ep_path))
...
def get_env(env_name,env_agent_n = None):
    # 动态导入环境（PettingZoo>=1.26 使用 mpe2，旧版使用 pettingzoo.mpe）
    module = import_mpe_module(env_name)
    ...
    if trick is None or not any(trick.values()):
        ...
        for key in trick.keys():
            if trick[key]:
                prefix += key + '_'
    # 查找现有的文件夹并确定下一个编号
    pattern = re.compile(rf'^{prefix}\d+') # ^ 表示开头，\d 表示数字，+表示至少一个
...
if __name__ == '__main__':
    ...
    # 训练数据导出（默认写入轨迹；需要关闭时用 --no_save_trajectory）
    _traj = parser.add_mutually_exclusive_group()
    _traj.add_argument(
        "--save_trajectory",
        dest="save_trajectory",
        action="store_true",
        help="Write trajectory CSV (default: on).",
    )
    _traj.add_argument(
        "--no_save_trajectory",
        dest="save_trajectory",
        action="store_false",
        help="Do not write trajectory CSV files.",
    )
    parser.set_defaults(save_trajectory=True)
    parser.add_argument(
        "--trajectory_format",
        type=str,
        choices=["default", "main_like", "json_per_step"],
        default="default",
        help="default: one row per env step, shared episode/global_step/episode_step + one JSON column per agent -> training_trajectory.csv; "
        "main_like: flat columns one row per agent-step -> training_trajectory_main_like.csv; "
        "json_per_step: same layout as default -> training_trajectory_agents_json.csv",
    )
    parser.add_argument(
        "--no_save_episode_csv",
        action="store_true",
        help="Skip training_episodes.csv (episode-level rewards and exploration scale)",
    )
    ...
    if args.save_trajectory:
        traj_fp, traj_writer, traj_format = _open_trajectory_writer(
            model_dir, env_agents, dim_info, traj_format=args.trajectory_format,
        )
    ...
    while episode_num < args.max_episodes:
        ...
        if traj_writer is not None:
            gstep = (step - 1) if traj_format in ("main_like", "default", "json_per_step") else step
            _write_trajectory_rows(
                traj_writer,
                traj_format,
                episode_num,
                gstep,
        ...
        if any(done.values()):
            ...
            if not args.no_save_episode_csv:
                ep_row = {
                    'episode': episode_num + 1,
                    'global_step': step,
                    'mean_reward': float(np.mean(list(episode_reward.values()))),
                    'exploration_scale': _current_exploration_scale(args, args.supplement, ou_noise, env_agents),
                }
                for agent_id in env_agents:
                    ep_row[f'reward_{agent_id}'] = float(episode_reward[agent_id])
    ...
    if traj_fp is not None:
        traj_fp.close()
    if not args.no_save_episode_csv and episode_rows:
        _save_training_episodes_csv(model_dir, episode_rows, env_agents)
